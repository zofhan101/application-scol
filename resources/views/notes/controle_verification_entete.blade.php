...
@section('title', 'Controle de la vérification des en-têtes')
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12">
                <div class="card info-card">
                    <div class="card-body">
                        <h5 class="card-title">Controle de la vérification des en-têtes</h5>
                            ...
                            <div class="card-body">
                              <!-- General Form Elements -->
                                <div class="modal-footer">
                                    ...
                                    <button type="button" class="btn btn-danger" data-bs-dismiss="modal" onClick="verrouiller_verification()">Confirmer</button>
                                </div>
                            </div>
                          </div>
                        </div>
...
<script>
    ...
    async function verrouiller_verification(){
        ...
        let url= "{{ route('verrouiller_verification_entete') }}";
            try {
    ...
    async function ouvrir_verification(){
        ...
        let url= "{{ route('ouvrir_verification_entete') }}";
            try {
                let response = await fetch(url, {
                    method: 'POST',
                    headers: {
        ...
        let url= "{{ route('get_operation_par_examen') }}";
            try {
                ...
                if(response.ok){
                    ...
                    else if(data.operation.date_cloture_saisie_en_tete != null && data.operation.date_ouverture_verification_en_tete == null){
                        console.log('Saisie cloturee, verification non ouverte');
                        let titre_cloture = document.getElementById('titre_ouverture');
                        titre_cloture.textContent = "Confirmez-vous l'\ouverture de la vérification des en-têtes pour : " + nom_examen_par_au;
                        liste_boutons[0].disabled = false;
                        liste_boutons[1].disabled = true;

                    }
                    else if(data.operation.date_ouverture_verification_en_tete != null && data.operation.date_cloture_verification_en_tete == null){
                        console.log('Vérification ouverte et non cloturée');
                        let titre_cloture = document.getElementById('titre_cloture');
                        titre_cloture.textContent = "Confirmez-vous la cloture de la vérification des en-têtes pour : " + nom_examen_par_au;
                        liste_boutons[0].disabled = true;
