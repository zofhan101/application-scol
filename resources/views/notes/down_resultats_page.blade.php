...
@section('title', 'Consultation des Résultats d\'examen')

@section('content')
<div class="pagetitle">
    <h1>Consultation des Résultats d\'examen</h1>
</div>
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="card">
              <div class="card-body">
                <!-- Vertical Form -->
                <form class="row g-3" action="{{ route('notes.down_resultats_specifique') }}" method="POST" id="form">
                    @csrf
                    ...
                    <div class="col-12">
                        <label for="inputAddress" class="form-label">Examen</label>
                            @error('id_examen_par_au')
                                <div class="alert alert-danger alert-dismissible fade show" role="alert">
                                    {{ $message }}
                                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                </div>
                            @enderror
                            <select class="form-select" aria-label="Default select example" id="examen" name="id_examen_par_au" disabled>
                            </select>
                  ...
                  <div class="text-center d-flex">
                    <button type="submit" class="w-50 btn btn-primary" id="one">Télécharger des résultats spécifiques</button>
                    <button type="submit" class="w-50 btn btn-primary" id="all">Télécharger tous les résultats</button>
...
<script>
    ...
    document.getElementById('all').addEventListener('click', function(){
        form.action = "{{ route('notes.down_resultats') }}";
        form.submit();
    });


    document.getElementById('one').addEventListener('click', function(){
        form.action = "{{ route('notes.down_resultats_specifique') }}";
        form.submit();
    });
    ...
    au.addEventListener('change', rafraichir_examens);

    rafraichir_niveaux();
    rafraichir_examens();

    async function rafraichir_examens(){
        //récupération des examens  correspondants a l'au sélectionné
        let url = "{{route('au.get_liste_examen')}}";
        id_au = au.value;
        let formdata = new FormData();
        formdata.append("id_au", id_au);
        try {
            ...
            let examens = document.getElementById('examen');
            if(response.ok){
                examens.disabled = false;
                examens.innerHTML = '';
                let examen;
                for(let i=0; i<data.examens.length; i++){
                    examen = data.examens[i];
                    let newOption = document.createElement('option');
                    newOption.value = examen.id_examen_par_au;
                    newOption.textContent = examen.nom_session_examen;
                    examens.appendChild(newOption);
                }
            }
            else{
                ...
                examens.disabled = true;
                examens.innerHTML = "";
                console.error('ERREUR  à la récupération des examens: ', data);
                ...
                if(data.errors  != undefined){
                    if(data.errors.id_au != undefined)
            ...
            examens.disabled = true;
            examens.innerHTML = "";
            console.error('ERREUR à la récupération des examens: ', error);
            alert('ERREUR à la récupération des examens:');
        }
    }
